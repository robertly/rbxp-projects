...
local mergeArraysUniqueOnly = require(ReplicatedStorage.Source.Utility.mergeArraysUniqueOnly)
...
function SessionLockedDataStoreWrapper._lockMetadata(
	...
	if not metadata.lockId or metadata.lockId ~= existingLockId then
		return Freeze.Dictionary.merge(metadata, { lockId = HttpService:GenerateGUID() })
	end
...
function SessionLockedDataStoreWrapper._unlockMetadata(self: ClassType, metadata: { [any]: any }): { [any]: any }
	return Freeze.Dictionary.merge(metadata, { lockId = Freeze.None })
end
...
function SessionLockedDataStoreWrapper._requestAsync(
	...
	local lockSettings = Freeze.Dictionary.merge(DEFAULT_LOCK_SETTINGS, optionalLockSettings or {}) :: LockSettings
