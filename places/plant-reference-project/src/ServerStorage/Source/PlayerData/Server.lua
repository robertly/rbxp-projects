...
local SessionLockedDataStoreWrapper = require(script.Parent.SessionLockedDataStoreWrapper)
...
local getMaxRequestTime = require(ServerStorage.Source.Utility.PlayerData.getMaxRequestTime)
local copyDeep = require(ReplicatedStorage.Source.Utility.copyDeep)

local ALLOW_STUDIO_ACCESS = true -- Set this to false to disable datastore access in studio
...
function PlayerDataServer.setMetaData(player: Player, metadata: { [string]: any })
	...
	PlayerDataServer._playerDataMetadata[player] = copyDeep(metadata)
end
...
function PlayerDataServer._onPlayerAddedAsync(player: Player)
	...
	if dataStoresEnabled then
		...
		if success then
			PlayerDataServer._playerData[player] = (result or copyDeep(PlayerDataServer._defaultData)) :: PlayerData
			PlayerDataServer._playerDataSynced[player] = copyDeep(PlayerDataServer._playerData[player]) :: PlayerData
		else
	...
	if hasErrored then
		...
		PlayerDataServer._playerData[player] = copyDeep(PlayerDataServer._defaultData) :: PlayerData
...
function PlayerDataServer._sendLoadedData(player: Player)
	...
	local data = Freeze.Dictionary.filter(
		copyDeep(PlayerDataServer._playerData[player]),
...
function PlayerDataServer._savePlayerDataAsync(player: Player, data: any, unlockAfter: boolean)
	...
	local dataSubmitted = copyDeep(data)
	...
	if player:IsDescendantOf(Players) then
		...
		Network.fireClient(Network.RemoteEvents.PlayerDataSaved, player, success, errorType)
	end
...
-- Returns if it is save to save the player's data
